...
# distributions may destroy the booting sequence.
#
...
if [[ "$MODEL" != *"Raspberry Pi 3"* ]]; then
    echo "ERROR: This script is ONLY for Raspberry Pi 3. Detected: $MODEL"
    exit 1
fi

# Configuration Paths
STATE_FILE="/var/lib/raspi3-fastboot/enabled"
BACKUP_ROOT="/var/lib/raspi3-fastboot/backups"
TS=$(date +%Y%m%d_%H%M)
...
if [[ -f "$STATE_FILE" ]]; then
    ...
    echo "Please run 'raspi3-fastboot-disable.sh' before running this again."
    exit 1
fi
...
# AUDIO ENABLED, WIFI & BLUETOOTH DISABLED (MODIFY)
# sed -i '/disable_splash=/d; /boot_delay=/d; /dtparam=audio=/d; /dtparam=sd_poll_once=/d' /boot/firmware/config.txt
sed -i '/disable_splash=/d; /boot_delay=/d; /dtparam=sd_poll_once=/d; /disable-wifi/d; /disable-bt/d; /vc4-kms-v3d/d' /boot/firmware/config.txt
cat <<EOF >> /boot/firmware/config.txt
# Added by raspi3-fastboot-enable.sh
disable_splash=1
boot_delay=0
# AUDIO ENABLED
#dtparam=audio=off
...
# Ensure KMS Overlay is active for Pygame KMSDRM
